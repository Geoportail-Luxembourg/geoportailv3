...
LAYER
  ...
  DATA "geometry FROM (SELECT geo.* FROM geodata.table AS geo WHERE ST_Contains((${mapfile_data_subselect} 'layer_name'), ST_SetSRID(geo.geometry, 2169))) as foo using unique id using srid=2169"
  # Example data for secured layer by role (without any area)
  #DATA "geometry FROM (SELECT geo.geom as geom FROM geodata.table AS geo WHERE %role_id% IN (${mapfile_data_noarea_subselect} 'layer_name')) as foo USING UNIQUE gid USING srid=2169"
  # Example data for public layer
  #DATA "geometry FROM (SELECT geo.geom as geom FROM geodata.table AS geo) AS foo USING UNIQUE gid USING srid=2169"
  METADATA
    "wms_title" "layer_name" # For WMS
    "wms_srs" "EPSG:2169" # For WMS
