...
class Authentication(object):
    ...
    def get_user_info(self):
        if self.request.user is not None :
            return {"login":self.request.user.username,"role":self.request.user.role.name,"role_id":self.request.user.role.id, "mail":self.request.user.mail, "sn":self.request.user.sn}
        return {}
