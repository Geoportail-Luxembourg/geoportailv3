...
function writeExports(exports, callback) {
  temp.open({prefix: 'exports', suffix: '.js'}, function(err, info) {
    ...
    log.verbose('geoportailv3', 'Writing exports: ' + info.path);
...
function getDependencies(config, exports, callback) {
  writeExports(exports, function(err, exportsPath) {
    ...
    log.info('geoportailv3', 'Parsing dependencies');
    var options;
    if (config.src) {
      options = {
        lib: config.src,
...
function build(config, paths, callback) {
  ...
  if (!options.compile) {
    log.info('geoportailv3', 'No compile options found.  Concatenating ' +
        paths.length + ' sources');
    concatenate(paths, callback);
  } else {
    log.info('geoportailv3', 'Compiling ' + paths.length + ' sources');
